...
function HeroCarousel() {
    ...
    return (
        <div className="relative w-full aspect-[16/9] sm:h-80 md:h-96 lg:h-[28rem] rounded-lg overflow-hidden">
                ...
                >
                    ...
                    <div className="absolute bottom-4 left-4 right-4">
                        ...
                        >
                            ...
                            >
                                <Link href="/products" className='flex items-center flex-row gap-1'>
            ...
            <button
                onClick={prevSlide}
                className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/30 hover:bg-white/50 text-white p-2 rounded-full backdrop-blur-sm transition-all sm:p-3"
            ...
            <button
                onClick={nextSlide}
                className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/30 hover:bg-white/50 text-white p-2 rounded-full backdrop-blur-sm transition-all sm:p-3"
    ...
    return (
        <div className="bg-gray-50 min-h-screen">
            ...
            <motion.div
                ...
                transition={{ duration: 0.5 }}
                className="bg-gradient-to-r from-red-600 to-red-700 text-white text-center py-2 px-4 text-sm sm:text-base font-medium"
            >
            ...
            <main className="container mx-auto px-4 sm:px-6 max-w-7xl">
                ...
                >
                    <div className="flex flex-col lg:flex-row gap-6">
                        {/* Category Sidebar */}
                        <motion.div
                            className="hidden lg:block bg-white w-full lg:w-64 xl:w-72 p-4 shadow-sm rounded-lg border border-gray-100"
                            initial={{ x: -20, opacity: 0 }}
                            animate={{ x: 0, opacity: 1 }}
                            transition={{ delay: 0.3 }}
                        >
                            <h3 className="font-bold text-lg mb-4 text-gray-800">Shop Categories</h3>
                            <ul className="space-y-2">
                                {['Phones', 'Computing', 'Electronics', 'Fashion', 'Home & Office', 'Appliances'].map((category, index) => (
                                    ...
                                    >
                                        <Link
                                            href={`/categories`}
                                            className="hover:bg-gray-50 flex items-center gap-2 text-base p-2 rounded-md transition-colors text-gray-700 hover:text-red-600"
                                        >
                                            <FiChevronRight className="h-4 w-4 text-red-500" />
                                            {category}
                                        </Link>
                                    </motion.li>
                                ))}
                        ...
                        {/* Hero Carousel */}
                        <motion.div
                            className="w-full bg-white p-2 sm:p-4 shadow-sm rounded-lg border border-gray-100"
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ delay: 0.3 }}
                        >
                ...
                {/* Flash Sale Section */}
                <motion.section
                    className="bg-white py-6 sm:py-8 shadow-sm border border-gray-100 rounded-lg my-6"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    ...
                    <div className="px-4 sm:px-6">
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-3">
                            <motion.h2
                                ...
                                className="text-xl sm:text-2xl font-bold flex items-center gap-2 text-gray-800"
                            >
                                <motion.span
                            ...
                            <div className="flex items-center gap-2 bg-red-50 px-3 py-1.5 rounded-full border border-red-100">
                                <span className="text-red-600 font-medium text-sm">Ends in:</span>
                                <CountdownTimer initialTime={2 * 60 * 60 + 45 * 60 + 30} />
                            </div>
                        </div>

                        <div className="grid  sm:grid-cols-3 md:grid-cols-4 gap-4 sm:gap-6">
                ...
                >
                    <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6 border border-gray-100">
                        <h2 className="text-xl sm:text-2xl font-bold mb-6 text-gray-800">Shop By Category</h2>
                        <div className="grid grid-cols-3 xs:grid-cols-4 sm:grid-cols-6 gap-4 sm:gap-6">
                            {categories.map((category, index) => (
                                ...
                                >
                                    <Link
                                        href={`/categories`}
                                        className="flex flex-col items-center p-3 sm:p-4 hover:bg-gray-50 rounded-lg transition-all hover:shadow-sm"
                                    >
                                        <motion.div
                                            whileHover={{ rotate: 10 }}
                                            className={`${category.color} p-3 sm:p-4 rounded-full mb-2`}
                                        >
                                            {category.icon}
                                        </motion.div>
                ...
                {/* Daily Deals */}
                <motion.section
                    className="bg-gray-50 py-8 sm:py-10 border-t border-gray-200 my-8 rounded-lg"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                ...
                >
                    <div className="px-4 sm:px-6">
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-3">
                            ...
                            >
                                <Link href="/deals" className='flex items-center flex-row'>
